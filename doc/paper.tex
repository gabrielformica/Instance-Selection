...
\subsection{Búsqueda local}
    ...
    \begin{algorithmic}
        \Function{BusquedaLocal}{$T: \text{Problema}$}
            \State $S \gets \text{solucionAleatoria()}$
            \While{\text{$\lnot condicionDeParada$}}
                \State $R \gets S$
...
\subsection{1NN}
    ...
    \begin{algorithmic}
        \Function{Calidad}{$S: \text{Solución}$, $T: \text{Problema}$}
            \State \text{Problema entrenamiento}
            \State \text{Problema resultado}
